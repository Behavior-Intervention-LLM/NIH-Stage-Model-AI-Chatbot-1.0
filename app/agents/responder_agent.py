...
"""
Responder Agent: final user-facing response generation.
All responses must go through the LLM.
RAG is supportive evidence, not a hard constraint.
"""
import json
from pathlib import Path
from typing import Any, List
...
class ResponderAgent(BaseAgent):
    """LLM-only response generation agent."""

    _FALLBACK_SECTIONS = {
        "system_global": """
You are an NIH Stage Model assistant.

Your job is to generate the final user-facing answer.

Core behavior:
1. Always answer the user's actual question directly.
2. Use retrieved evidence as support or refinement, not as a hard limit.
3. If retrieved snippets are incomplete, still provide the best complete answer from your general knowledge.
4. Do NOT make "the snippet does not mention..." the main answer unless the user explicitly asks about what a specific document says.
5. If stage confidence is low or stage is unknown, say so clearly and ask focused follow-up questions.
6. If the user asks for stage identification, explicitly state the most likely stage and briefly explain why.
7. If the user asks a definition/list question, answer directly and cleanly; do not drift into case-specific analysis.
8. Do not expose internal workflow, raw slots, JSON fields, or execution trace.
9. Match the user's language when obvious.
10. Output plain text only. No JSON. No markdown code fences.

Grounding policy:
- Prefer newer or revised retrieval evidence when it is relevant.
- If retrieval conflicts with your general knowledge, prefer the retrieval only when it is clearly about a revised or version-specific definition.
- If retrieval is partial, incomplete, or only tangentially relevant, answer the question normally and optionally mention the evidence briefly.

Style:
- Professional, concise, clear.
- Do not sound like a search engine or document parser.
- Do not say "based on the provided snippet" unless that framing is truly necessary.
""".strip(),

        "user_instruction_definition": """
This is a definition/list question.

Answer in this structure:
1. A direct one-sentence definition
2. Number of stages
3. Stage names
4. One-line description of each stage

Important:
- Do not make missing snippet details the main answer.
- If evidence is incomplete, still answer fully from general knowledge.
""".strip(),

        "user_instruction_stage": """
This is a stage-identification or stage-reasoning question.

Answer in this structure:
1. Most likely stage (explicitly write Stage 0 / Stage I / Stage II / Stage III / Stage IV / Stage V when possible)
2. Brief reasoning
3. Key uncertainty or missing information, if any
4. Focused follow-up question(s) only if needed
""".strip(),

        "user_instruction_general": """
This is a general NIH Stage Model question.

Answer directly and clearly.
Use retrieval evidence when useful, but do not let incomplete snippets block a complete answer.
""".strip(),
    }
    ...
    @staticmethod
    def _parse_responder_markdown(text: str) -> dict[str, str]:
        """Split prompts/responder.md on level-2 headings: ## section_name"""
        sections: dict[str, str] = {}
        current: str | None = None
        buf: list[str] = []
    ...
    def _collect_evidence(state: SessionState) -> tuple[List[str], List[str], List]:
        ...
        for artifact in state.artifacts[-8:]:
            ...
            if isinstance(artifact.content, str):
                ...
                if "no matching" not in content_lower and "not found" not in content_lower:
                    evidence_lines.append(artifact.content[:700])

        return evidence_lines, evidence_sources, citations

    @staticmethod
    def _workflow_structured_excerpt(structured: Any, max_chars: int = 3500) -> str:
        if not structured:
            return "{}"
        try:
            raw = json.dumps(structured, ensure_ascii=False, indent=2)
    ...
    @staticmethod
    def _detect_query_mode(user_message: str, state: SessionState) -> str:
        message_lower = user_message.lower().strip()
        intent_payload = state.slots.extracted_features.get("intent_payload", {}) or {}
        intent_query_type = str(intent_payload.get("query_type", "")).lower()

        is_definition_query = (
            any(k in message_lower for k in ["what is", "what's", "define", "how many stages", "number of stages", "list stages", "explain"])
            and any(k in message_lower for k in ["nih stage model", "nih stage", "stage model"])
        ) or intent_query_type == "definition"

        is_stage_query = intent_query_type in {"stage_classification", "stage_requirements", "next_step"} or (
            "stage" in message_lower and any(k in message_lower for k in ["which", "what", "identify", "classification", "requirements", "criteria"])
        )
    ...
    def _build_user_prompt(
        ...
        if mode == "definition":
            task_instruction = sections["user_instruction_definition"]
        elif mode == "stage":
            task_instruction = sections["user_instruction_stage"]
        else:
            task_instruction = sections["user_instruction_general"]
        ...
        lines = [
            ...
            task_instruction,
            "",
            "Relevant context for synthesis:",
            f"- Inferred stage: {state.slots.stage!r}",
            f"- Stage confidence: {state.slots.stage_confidence!r}",
            f"- Workflow: {xf.get('workflow', 'navigator')}",
            f"- Workflow summary: {xf.get('workflow_summary') or ''}",
            f"- Planner outline: {xf.get('planner_outline')}",
            f"- Next question from upstream: {xf.get('next_question')}",
            f"- Stage reasoning summary: {xf.get('reasoning_summary')}",
            f"- Missing stage info: {xf.get('missing_info')}",
            f"- Missing intent info: {xf.get('intent_missing_info')}",
            f"- Clarifying question (stage): {xf.get('clarifying_question')}",
            f"- Clarifying question (intent): {xf.get('intent_clarifying_question')}",
            f"- Guardrail warnings: {xf.get('guardrail_warnings') or []}",
            f"- Stage uncertain hint: {xf.get('stage_uncertain_hint', False)}",
            "",
            f"RAG active: {rag_active}",
            f"Knowledge sources: {evidence_sources}",
            "Evidence snippets:",
        ...
        if evidence_lines:
            for i, snippet in enumerate(evidence_lines[:5], 1):
        ...
        lines.extend(
            [
                "",
                "Workflow structured output (may be partial):",
                self._workflow_structured_excerpt(xf.get("workflow_structured_output") or {}),
                "",
                ...
                "- If retrieval is incomplete, still answer using general knowledge.",
                "- If clarification is required, keep it focused and minimal.",
            ]
        )
    ...
    def run(self, state: SessionState, user_message: str, context: str = "") -> AgentOutput:
        if not llm_client.is_enabled():
            return AgentOutput(
                decision={},
                confidence=0.15,
                analysis="LLM disabled; responder is configured as LLM-only",
                user_facing=(
                    "The responder is configured to use the language model for all replies, "
                    "but the LLM client is currently disabled. Please check your provider/API configuration."
                ),
                metadata={},
            )

        return self._run_with_llm(state, user_message, context)
